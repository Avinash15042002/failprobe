...
Thanks for your interest in improving AgentProbe! This guide covers the dev
...
```bash
ruff check agentprobe/ api/        # zero issues
...
- **Never block the agent.** Any failure inside AgentProbe is caught, logged to
...
- **The rule-based classifier makes zero LLM/network calls.** `agentprobe/classifier/`
...
|---|---|---|
| `agentprobe/classifier/` | Rule-based classification | LLM calls, DB, network |
| `agentprobe/evaluator/` | LLM judge, golden dataset, meta-eval | Span capture, classification |
| `agentprobe/storage/` | ORM models, sessions, migrations | Business logic |
| `agentprobe/regression/` | Stats, CI runner, baseline | Span capture, evaluation |
| `api/` | HTTP routes and schemas | Business logic (import from the package) |
| `agentprobe/cli/` | Commands, rich output | Direct DB access (go via API or package) |
