"""The ``@probe`` decorator — AgentProbe's primary user-facing API.

``probe`` wraps an ``async`` agent function and, around each call, captures an
:class:`~agentprobe.models.AgentSpan`, classifies any failure with the
rule-based :class:`~agentprobe.classifier.FailureClassifier`, and emits the span
...
* **Never block, never crash.** Span emission is always scheduled with
  ``asyncio.create_task`` and never awaited. Any error inside AgentProbe's own
...
from agentprobe.models import AgentSpan
from agentprobe.tracer import _emit_span
...
def probe(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            # 6-8. Build span, classify, emit — fully isolated so an AgentProbe
            ...
            try:
                ...
                logger.error(
                    "AgentProbe internal error while recording span", exc_info=True
                )
