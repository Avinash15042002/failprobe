"""Canonical in-memory dataclasses shared across the FailProbe pipeline.

These are pure Python ``dataclasses`` — not Pydantic schemas and not SQLAlchemy
ORM models. The ORM equivalents live in ``failprobe/storage/models.py`` (Task 06).
...
``failprobe/classifier/`` and thereby avoid a circular import. The classifier
