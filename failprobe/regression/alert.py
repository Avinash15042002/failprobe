...
The webhook URL is conventionally passed from the ``FAILPROBE_SLACK_WEBHOOK``
...
def _format_message(result: RegressionResult) -> str:
    ...
    lines = ["🚨 FailProbe Regression Detected", f"Suite: {result.suite_name}"]
...
async def send_slack_alert(webhook_url: str, result: RegressionResult) -> None:
    ...
    unconfigured ``FAILPROBE_SLACK_WEBHOOK`` never breaks the run. Any HTTP error
    is logged and swallowed — alerting must never fail the build.

    Args:
        webhook_url: Slack incoming-webhook URL (typically from
            ``FAILPROBE_SLACK_WEBHOOK``).
        result: The confirmed-regression result to describe.
    """
    if not webhook_url:
        logger.warning("FAILPROBE_SLACK_WEBHOOK not set; skipping regression alert.")
        return
