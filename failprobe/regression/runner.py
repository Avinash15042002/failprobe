...
from failprobe.regression.baseline import DEFAULT_BASELINE_NAME, load_baseline
...
DEFAULT_REPORT_PATH = "failprobe_report.json"
...
async def snapshot_baseline(
    ...
    Returns:
        The persisted :class:`~failprobe.storage.models.RegressionBaseline` row.
    """
    from failprobe.regression.baseline import save_baseline
...
def _build_arg_parser() -> argparse.ArgumentParser:
    """Build the ``python -m failprobe.regression.runner`` argument parser.

    A convenience entry point only — the user-facing ``probe run`` CLI is
    TASK 15.
    """
    parser = argparse.ArgumentParser(description="Run an FailProbe regression suite.")
