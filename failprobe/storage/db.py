"""Async database engine factory and session management for FailProbe.

Supports SQLite (dev, zero config) and PostgreSQL (prod) via a single
connection string. The string is resolved from the ``FAILPROBE_DB_URL``
...
from failprobe.storage.models import Base
...
def _resolve_db_url() -> str:
    ...
    Precedence: the ``FAILPROBE_DB_URL`` environment variable, then
    ``ProbeConfig.db_url``. Returns the connection string passed to the async
    engine.
    """
    return os.environ.get("FAILPROBE_DB_URL") or get_config().db_url
