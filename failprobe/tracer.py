"""Async, batched span emission pipeline for FailProbe (Task 08).
...
def _print_console(span: AgentSpan) -> None:
    ...
    if span.success:
        line = (
            f"[failprobe] run={prefix} agent={span.agent_name} "
...
async def _emit_span(span: AgentSpan) -> None:
    ...
    try:
        ...
        logger.error("FailProbe error while emitting span", exc_info=True)
...
async def _flush_worker() -> None:
    ...
    while True:
        try:
            ...
            logger.error("FailProbe flush worker error", exc_info=True)
