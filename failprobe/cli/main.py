"""FailProbe CLI — six commands wrapping the API and the package layers.
...
from failprobe.evaluator.meta_eval import MetaEvalReport, run_meta_eval
from failprobe.regression.baseline import DEFAULT_BASELINE_NAME, list_baselines
from failprobe.regression.runner import (
    DEFAULT_REPORT_PATH,
    RegressionResult,
    run_suite,
    snapshot_baseline,
...
app = typer.Typer(
    help="FailProbe CLI — failure classification and regression CI for LLM agents",
...
def _api_base_url() -> str:
    """Return the API base URL (``FAILPROBE_API_URL`` env, or the default)."""
    return os.environ.get("FAILPROBE_API_URL", DEFAULT_API_URL).rstrip("/")


def _api_get(path: str, params: Optional[dict] = None) -> dict:
    """GET ``path`` from the FailProbe API and return the decoded JSON body."""
...
def _api_post(path: str, payload: dict) -> dict:
    """POST ``payload`` to ``path`` on the FailProbe API; return the JSON body."""
    with httpx.Client(base_url=_api_base_url(), timeout=30.0) as client:
